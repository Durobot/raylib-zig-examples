...
pub fn main() void
{
    ...
    var rec = c.Rectangle{ .x = @intToFloat(f32, c.GetScreenWidth()) / 2.0, .y = -100, .width = 100, .height = 100 };
    ...
    while (!c.WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        switch (state)
        {
            0 =>      // Move box down to center of screen
            {
                ...
                rec.y = c.EaseElasticOut(@intToFloat(f32, frames_counter), -100, @intToFloat(f32, c.GetScreenHeight()) / 2.0 + 100, 120);
            ...
            {
                frames_counter += 1;
                rec.height = c.EaseBounceOut(@intToFloat(f32, frames_counter), 100, -90, 120);
                rec.width = c.EaseBounceOut(@intToFloat(f32, frames_counter), 100, @intToFloat(f32, c.GetScreenWidth()), 120);
            ...
            {
                frames_counter += 1;
                rotation = c.EaseQuadOut(@intToFloat(f32, frames_counter), 0.0, 270.0, 240);
            ...
            {
                frames_counter += 1;
                rec.height = c.EaseCircOut(@intToFloat(f32, frames_counter), 10, @intToFloat(f32, c.GetScreenWidth()), 120);
            ...
            {
                frames_counter += 1;
                alpha = c.EaseSineOut(@intToFloat(f32, frames_counter), 1.0, -1.0, 160);
        ...
        {
            rec = .{ .x = @intToFloat(f32, c.GetScreenWidth()) / 2.0, .y = -100.0, .width = 100.0, .height = 100.0 };
            rotation = 0.0;
            alpha = 1.0;
            state = 0;
            frames_counter = 0;
